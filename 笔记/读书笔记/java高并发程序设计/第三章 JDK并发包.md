...
可以多次加入同一个锁，如果不是这样就会造成死锁的情况，但是问题也来了，当重入锁释放的少了，锁就会释放不了，其他的进入不了临界区，释放掉了会出现异常。
...
死锁的两种做法，一种是中断锁，一种是锁的尝试时间。

![1567900947088](phto/1567900947088.png)
...
信号量Semaphore,他为多线程提供了强大的控制，之前的syn或者是重入，每次只能有一个在执行，但是信号量可以有多个线程同时访问。
...
![1567909165789](phto/1567909165789.png)

- 使用

![1567909246101](phto/1567909246101.png)
...
![1567909984187](phto/1567909984187.png)

![1567910050442](phto/1567910050442.png)

![1567910108485](phto/1567910108485.png)

![1567910251970](phto/1567910251970.png)
...
![1567910306274](phto/1567910306274.png)
...
线程可以帮助我们完成任何，但是不加以限制有可能导致资源耗尽，比如oom异常，也有可能造成GC加长。线程在一定范围内可以提高效率，如果高于就会导致内存大量使用，拖垮应用。
...
这个的使用，如果这样使用是不合理的，所以还是使用一个线程，如果多次点击之后，就会判断最后一次的url和当前查询的是否相同，相同就返回，不相同就再来一次。
...
![1567930897235](phto/1567930897235.png)
...
- 线程池的执行过程
  ...
  ![1567931056235](phto/1567931056235.png)

  ![1567931088090](phto/1567931088090.png)
