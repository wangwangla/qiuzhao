...
## 硬件效率造成的问题
```
    一个任务需要使用CPU进行计算，一个用于磁盘数据加入到主内存，但是内存读写和CPU速度的差异，所以需要一个高速的缓存来处理，CPU计算与缓存交互。除了缓存，还有一个就是在使用的过程还可以对其进行重新排序。

    问题：缓存一致性问题【每个处理器为了效率都会有一个缓存，但是缓存中的数据都来自于同一块内存】

    处理器在为了更好的使用资源，会将内存进行乱序优化，将代码进行重组，保证结果与顺序执行是一样的结果，也就是编译时期指令重排序优化。这个时候如果后面的计算依赖于前面的计算，那么就会出现问题。
```
## 内存模型
```
java虚拟机屏蔽这一操作系统和硬件之间的访问差异，可以使得Java程序可以在各种平台上游相同的访问结果。比如c语言等在不同平台的运行结果会与差异。
```
## 主内存和工作内存
```
...
java内存模型是将规定其将数据放入内存，和将内存中的数据取出。他仅仅包括静态变量和类字段，不包括局部变量等这些东西。
